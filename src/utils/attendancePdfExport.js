...
const inferProgram = (course = {}) => {
  ...
  if (searchable.includes("ECONOMICS")) {
    return "B.Sc. (Hons.) in Economics";
  }
...
const normalizeDepartment = (value) => {
  ...
  if (/^CSE$/i.test(withoutUniversity)) {
    return DEFAULT_DEPARTMENT;
  }
  if (/^Department\s+of\s+/i.test(withoutUniversity)) {
    return `${withoutUniversity}, BUBT`;
  }
...
const makeFileName = ({ course = {}, teacher = {} }) => {
  const parts = [
...
const buildStudentRows = ({ sessions, dateGroups, students, matrix, rows }) => {
  ...
  return students.map((student) => {
    ...
    // One PDF column represents one date. The cell contains the number of
    // periods attended on that date; zero attendance is shown as "A".
    const attendance = dateGroups.map((dateGroup) => {
      const presentOnDate = dateGroup.sessions.reduce((count, session) => {
        return count + (Boolean(matrix?.[roll]?.[session.key]) ? 1 : 0);
      }, 0);
    ...
    const percentage = totalClasses > 0 ? Math.ceil((totalPresent / totalClasses) * 100) : 0;
...
export const createAttendancePdf = ({ data, computed, teacherFallback = {} }) => {
  ...
  // The PDF uses one column per date, regardless of how many periods were
  // held on that date. Switch orientation based on the number of date columns.
  ...
  const drawPageHeading = () => {
    ...
    doc.text(rightLabel, pageWidth - 2, 113, { align: "right" });

    doc.setFont("times", "normal");
    const courseLabel = `${safeText(course.title)} (${safeText(course.code)})`;
    const maxCourseWidth = Math.max(40, pageWidth - courseLabelStart - rightWidth - 18);
    const fitted = fitText(doc, courseLabel, maxCourseWidth, 9.5, 7);
    doc.setFontSize(fitted.size);
    doc.text(fitted.text, courseLabelStart, 113);
  };
  ...
  const drawTableHeader = (y) => {
    ...
    columns.forEach((column) => {
      ...
      if (column.type === "id") {
        ...
        doc.text(label, x + column.width / 2 + 2.5, y + headerHeight - 3, {
          align: "left",
          angle: 90,
        });
      }
  ...
  const drawNotesRow = (y) => {
    ...
    columns.forEach((column) => {
      ...
      if (column.type === "id") {
        doc.setFont("times", "bold");
        doc.setFontSize(8.5);
        doc.text("Notes", x + column.width / 2, y + notesHeight / 2 + 3, {
          align: "center",
        });
      }
      x += column.width;
  ...
  const drawStudentRow = (row, y) => {
    ...
    columns.forEach((column) => {
      ...
      if (column.type === "id") {
        ...
        doc.setFontSize(fitted.size);
        doc.text(fitted.text, x + column.width / 2, y + rowHeight / 2 + 2.7, {
          align: "center",
        });
      } else if (column.type === "name") {
        doc.setFont("times", "normal");
        ...
        doc.text(value, x + column.width / 2, y + rowHeight / 2 + 2.5, {
          align: "center",
        });
      }
  ...
  const firstPageCapacity = Math.max(
    1,
    Math.floor(
      (pageHeight - tableTop - headerHeight - notesHeight - bottomMargin) / rowHeight
    )
  );
  ...
  const signatureName = teacher.shortCode
  ...
  return {
    doc,
    filename: makeFileName({ course, teacher }),
  };
};
